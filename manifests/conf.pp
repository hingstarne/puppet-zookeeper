#
# = Define: zookeeper::conf
#
# With this define you can manage any zookeeper configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/zookeeper/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/zookeeper/my.conf',
#
# [*ensure*]
...
define zookeeper::conf (
  ...
  $manage_path = $path ? {
    undef   => "${zookeeper::config_dir_path}/${name}",
    default => $path,
  }
  ...
  $manage_mode = $mode ? {
    undef   => $zookeeper::config_file_mode,
    default => $mode,
  }

  $manage_owner = $owner ? {
    undef   => $zookeeper::config_file_owner,
    default => $owner,
  }

  $manage_group = $group ? {
    undef   => $zookeeper::config_file_group,
    default => $group,
  }

  $manage_require = $require ? {
    undef   => $zookeeper::config_file_require,
    default => $require,
  }

  $manage_notify = $notify ? {
    undef   => $zookeeper::manage_config_file_notify,
    default => $notify,
  }

  $manage_replace = $replace ? {
    undef   => $zookeeper::config_file_replace,
    default => $replace,
  }

  file { "zookeeper_conf_${name}":
    ensure  => $ensure,
    source  => $source,
